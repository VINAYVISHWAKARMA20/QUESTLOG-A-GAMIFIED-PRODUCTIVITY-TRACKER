...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String COLUMN_QUEST_PRIORITY = "priority"; // 1=LOW, 2=MEDIUM, 3=HIGH, 4=CRITICAL
    ...
    private static final String CREATE_TABLE_QUESTS = "CREATE TABLE " + TABLE_QUESTS + "("
            ...
            + COLUMN_QUEST_PRIORITY + " INTEGER DEFAULT 2"
            + ")";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 5) {
            db.execSQL("ALTER TABLE " + TABLE_QUESTS + " ADD COLUMN " + COLUMN_QUEST_PRIORITY + " INTEGER DEFAULT 2");
        }
    ...
    public long addQuest(String title, String details, String category, String date, String time, int difficulty, String recurrence) {
        return addQuestWithPriority(title, details, category, date, time, difficulty, recurrence, TaskScheduler.PRIORITY_MEDIUM);
    }

    public long addQuestWithPriority(String title, String details, String category, String date, String time, int difficulty, String recurrence, int priority) {
        SQLiteDatabase db = this.getWritableDatabase();
        android.content.ContentValues values = new android.content.ContentValues();
        ...
        values.put(COLUMN_QUEST_PRIORITY, priority);
    ...
    public List<Quest> getAllQuests() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                int priority = TaskScheduler.PRIORITY_MEDIUM;
                int priorityIndex = cursor.getColumnIndex(COLUMN_QUEST_PRIORITY);
                if (priorityIndex != -1) {
                    priority = cursor.getInt(priorityIndex);
                    if (priority == 0) priority = TaskScheduler.PRIORITY_MEDIUM;
                }
                
                quests.add(new Quest(cursor.getInt(0), cursor.getString(1), cursor.getString(2),
                        cursor.getString(3), cursor.getString(4), cursor.getString(5),
                        cursor.getInt(6), cursor.getString(7), recurrence, priority));
            } while (cursor.moveToNext());
        }
        cursor.close();
        return quests;
    ...
    public void updateQuestSchedule(int id, String date, String time) {
        SQLiteDatabase db = this.getWritableDatabase();
        android.content.ContentValues values = new android.content.ContentValues();
        values.put(COLUMN_QUEST_DATE, date);
        values.put(COLUMN_QUEST_TIME, time);
